...
def _generate_synthetic_packetflow_csv() -> str:
    """
    Creates a realistic-ish packet flow dataset (mixed benign + DDoS burst).
    Format: timestamp,src_ip,dst_ip,protocol,length,flags
    """
    now = datetime.utcnow().replace(microsecond=0)
    victim = "10.0.0.5"

    lines = []
    # Benign baseline traffic (small)
    benign_sources = [f"192.168.1.{i}" for i in range(10, 25)]
    for i in range(180):
        ts = now + timedelta(seconds=i // 6)  # spread out
        src = random.choice(benign_sources)
        dst = random.choice([victim, "10.0.0.8", "10.0.0.9"])
        proto = random.choice(["TCP", "UDP", "DNS"])
        length = random.choice([60, 74, 120, 220, 512])
        flags = "ACK" if proto == "TCP" else ""
        lines.append(f"{ts.isoformat()},{src},{dst},{proto},{length},{flags}")

    # DDoS-like burst to victim (many sources, SYN heavy)
    ddos_sources = [f"203.0.113.{i}" for i in range(1, 120)]  # TEST-NET-3
    for i in range(2200):
        ts = now + timedelta(seconds=(i // 600))  # concentrated window
        src = random.choice(ddos_sources)
        dst = victim
        proto = "TCP"
        length = random.choice([60, 60, 60, 74])  # small packets typical of SYN
        flags = "SYN"
        lines.append(f"{ts.isoformat()},{src},{dst},{proto},{length},{flags}")
