...
def create_model():
    ...
    checkpointer = ModelCheckpoint(filepath='../models/best_lc_model.hdf5', verbose=2, save_best_only=True, save_weights_only=True)
    checkpoint_cb = ModelCheckpoint('../models/xcp_lc_model.h5', save_best_only=True, monitor='val_accuracy',
                                    mode='max', verbose=1)
    ...
    plt.ylabel("True")
    plt.savefig("../static/vis/xcp_cnf.jpg")
    plt.close()
    ...
    plt.close()
    """
    # Save training history for debugging
    import json
    with open("../static/vis/xcp_training_history.json", "w") as f:
    ...
    if val_acc:
        ...
        print("⚠️ Warning: Validation accuracy not found in training history.")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    ...
    if val_loss:
        ...
        print("⚠️ Warning: Validation loss not found in training history.")
    ...
    model.save('../models/trained_lung_cancer_model.h5')
